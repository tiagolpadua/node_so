(function () {
    ...
    function dispatcher(listaProcessos, filaProcessosProntos, memoriaVirtual) {
        var freePid = 1;
        var listaProcessosProntos, listaProcessosExecutando, processoExecutando;
        /*
        ...
        //Cada passagem no loop é um tick do processador
        while(listaProcessos.length > 0){
            if (!processoExecutando) {
                listaProcessosProntos = getProcessoPorStatus(listaProcessos, STATUS.PRONTO);

                //FIXME: Tratar caso de não haverem processos prontos
